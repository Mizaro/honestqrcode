(function () {
  ...
  let activeType = TYPE_BY_ID[requestedType] ? requestedType : 'link';
  ...
  function fieldsFor(type) {
    ...
    const fieldMap = {
      ...
      phone: `<label for="phone-code">Country code</label>${cc('phone-code')}<label for="phone-number">Phone number</label><input id="phone-number" inputmode="tel" placeholder="5551234567">`,
      sms: `<label for="sms-code">Country code</label>${cc('sms-code')}<label for="sms-number">Phone number</label><input id="sms-number" inputmode="tel"><label for="sms-body">Message</label><textarea id="sms-body" rows="2"></textarea>`,
      whatsapp: `<label for="wa-code">Country code</label>${cc('wa-code')}<label for="wa-number">Phone number</label><input id="wa-number" inputmode="tel"><label for="wa-message">Message (optional)</label><textarea id="wa-message" rows="2"></textarea>`,
  ...
  function buildPayload() {
    ...
    if (type === 'wifi') {
      ...
      if (encryption !== 'nopass' && !password) throw new Error('Enter the WiFi password, or choose Open security.');
      const passwordSegment = encryption === 'nopass' ? '' : `;P:${escapeWifi(password)}`;
      return `WIFI:T:${encryption};S:${escapeWifi(ssid)}${passwordSegment}${hiddenSuffix};;`;
    }
    ...
    if (type === 'event') {
      ...
      const lines = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Honest QR Code//Event QR//EN',
        'BEGIN:VEVENT',
    ...
    if (type === 'social') {
      ...
      return `${bases[valueOf('social-platform')] || bases.instagram}${encodeURIComponent(handle)}`;
    }
  ...
  function renderQuickTypes() {
    quickTypes.innerHTML = QUICK_TYPES.map((id) => {
      ...
      return `<button type="button" class="quick-type${id === activeType ? ' is-active' : ''}" data-type="${id}" aria-pressed="${id === activeType}">${type.label}</button>`;
    }).join('');
  ...
  function bindCustomizeInputs() {
    ...
    $('logo-file').addEventListener('change', (event) => {
      ...
      if (!allowedTypes.includes(file.type) || file.size > 2 * 1024 * 1024) {
        event.target.value = '';
        logoState = { image: null, dataUrl: '' };
        statusEl.textContent = 'Choose a PNG, JPG, WebP, or SVG logo smaller than 2 MB.';
        statusEl.className = 'status status-warn';
        return;
      }
      ...
      reader.onload = () => {
        ...
        image.onerror = () => {
          event.target.value = '';
          logoState = { image: null, dataUrl: '' };
          statusEl.textContent = 'That logo could not be read as an image.';
          statusEl.className = 'status status-warn';
        };
      ...
      reader.onerror = () => {
        event.target.value = '';
        statusEl.textContent = 'That logo file could not be read.';
        statusEl.className = 'status status-warn';
      };
  ...
  function downloadCurrentCanvas(format) {
    ...
    canvas.toBlob((blob) => {
      if (!blob) {
        statusEl.textContent = 'The image could not be prepared. Try SVG instead.';
        statusEl.className = 'status status-warn';
        return;
      }
